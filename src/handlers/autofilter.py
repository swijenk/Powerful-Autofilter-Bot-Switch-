import logging
import re
from swibots import BotApp, BotContext, CommandEvent, InlineKeyboardButton, InlineMarkup, Message, filters, CallbackQueryEvent, MessageEvent, Media
        ...
        if (len(extracted) >= 2) and not message.replied_to:
            ...
            try:
                ...
                reply_text, btn, alert = parser(
                    message.replied_to.message, text, app)
            except:
                reply_text = ""
                btn = "[]"
                alert = None
                            ...
                            if btn == "[]":
                                await message.reply_text(
                                    text.format(
                                        ...
                                        username=None if not message.user.username else '@' + message.user.username,
                                        id=message.user.id,
                                        query=name
                                    ),
                                )
